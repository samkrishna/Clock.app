...
- (id)initWithFrame:(NSRect)frame 
{
    ...
    [self clockStateChanged:nil];
	
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(clockStateChanged:) name:ClockStateChangedNotification object:nil];
...
- (void)drawRect:(NSRect)rect 
{
    ...
    NSString *formatString = (state == NSOffState) ? @"%I:%M:%S %p" : @"%H:%M:%S";
	[[now descriptionWithCalendarFormat:formatString timeZone:nil locale:nil] drawInRect:timeRect withAttributes:self.textAttributes];
	[[now descriptionWithCalendarFormat:@"%x" timeZone:nil locale:nil] drawInRect:dateRect withAttributes:self.dateAttributes];
}

- (void)clockStateChanged:(NSNotification *)notification
{
    // Configure current time
    NSInteger state = [[NSUserDefaults standardUserDefaults] integerForKey:kCLKStateKey];
    NSString *formatString = (state == NSOffState) ? @"%I:%M:%S %p" : @"%H:%M:%S";
    NSString *clockDisplay = [[NSDate date] descriptionWithCalendarFormat:formatString timeZone:nil locale:nil];
